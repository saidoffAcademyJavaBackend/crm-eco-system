...
import uz.saidoff.crmecosystem.payload.WarningResponseDto;
...
public class EmployeeMapper {
  ...
  public EmployeeDto toDto(User user) {
    ...
    var warnings = userService.getUserCurrentWarnings(user.getId());
    int warningCount = warnings.size();

    Optional<Warning> reduce = warnings.stream().filter(Warning::isPunishment).findAny();
    WarningResponseDto warningDto = new WarningResponseDto(warningCount, 2 > warningCount ? null : reduce.get().getId());

    employeeDto.setWarningDto(warningDto);
